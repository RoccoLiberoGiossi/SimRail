...
def process_single_wheel(args):
    """Runs in a separate process. Must be a top-level function."""
    (
        index,
        wheel,
        rail,
    ...
    y_rail = rail_y(t)
    rail.calculate_position(y_rail, 0)

    contact_eq.patch_search(wheel, rail, material_model, discretization=discratization)

    # --- Per-patch processing ---
    local_Y_forces = []
    local_Q_forces = []
    creep_xs, creep_ys, phi_s, delta_rs, centroids = [], [], [], [], []

    local_F = np.zeros(6)

    for contacts in contact_eq.contact_patches:
        ...
        if type(contacts.centroid_wheel) != int:
            ...
            local_F[0] += force_vector[0]
            local_F[1] += force_vector[1]
            local_F[2] += force_vector[2]
            local_F[3] += contact_radius * force_vector[0]
            local_F[4] += contacts.centroid_wheel.item() * force_vector[0]
            local_F[5] += contacts.centroid_wheel.item() * force_vector[2]

            local_Y_forces.append(contacts.Y_force)
            local_Q_forces.append(contacts.Q_force)
    ...
    return local_F, local_Y_forces, local_Q_forces, y_rail
...
def dynamics(
    ...
    # --- Build args for each wheel ---
    args_list = [
        (i, wheels[i], rails[i], contact_eq_s[i], material_model,
        tangent_method, kalker_tables, new_state, t, v, q, 10, rail_y)
        for i in range(len(wheels))
    ]

    # --- Run in parallel ---
    # with ProcessPoolExecutor(max_workers=len(wheels)) as executor:
    results = list(executor.map(process_single_wheel, args_list))

    # --- Merge results on main process ---
    F_vector = np.zeros(6)
    Y_forces, Q_forces = [], []

    print(f"Results from parallel processes: {results}")

    for local_F, local_Y, local_Q, y_rail in results:
        F_vector += local_F
        Y_forces.extend(local_Y)
        Q_forces.extend(local_Q)
    ...
    _ax_1.plot(
        wheels[1].wheel_profile_pos[:, 0], wheels[1].wheel_profile_pos[:, 1], c="black"
    )
    _ax_1.plot(rails[0].rail_profile_pos[:, 0], rails[0].rail_profile_pos[:, 1], c="r")
    _ax_1.plot(
        wheels[0].wheel_profile_pos[:, 0], wheels[0].wheel_profile_pos[:, 1], c="black"
    )
    _ax_1.set_title(f"Wheel {t} s")
    _ax_1.set_aspect("equal")
...
if __name__ == "__main__":
    ...
    ds = 0.025e-3  # max displacement
    ramp_start = len(t_eval) // 64
    ramp_end = len(t_eval) // 2
    ramp_length = ramp_end - ramp_start

    rail_y = np.zeros_like(t_eval)
    rail_y[ramp_start:ramp_end] = ds * np.linspace(0, 1, ramp_length)
    rail_y[ramp_end:] = ds
    ...
    executor = ProcessPoolExecutor(max_workers=len(wheels))
