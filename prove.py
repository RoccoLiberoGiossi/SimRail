...
def dynamics(
    ...
    Y_forces_series.append(Y_forces[0])
    ...
    global _fig, _ax_1, _ax_2

    # Create the figure only once
    if _fig is None:
        # _fig, _ax_1 = plt.subplots(1, 1, figsize=(10, 4))
        _fig, (_ax_1, _ax_2) = plt.subplots(2, 1, figsize=(10, 4))
...
rail_y[50:len(t_eval)//2] = 0.1e-3*np.arange(len(t_eval)//2-50)/(len(t_eval)//2-50)
rail_y[len(t_eval)//2:] = 0.1e-3*np.flip(np.arange(len(t_eval)//2+1))/(len(t_eval)//2+1)
